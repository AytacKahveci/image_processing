...
/*
Controlled movement in XY plane
*/
#include <ros/ros.h>
...
public:
    ...
    ObjectPositioning(ros::NodeHandle& nh) : nh_(nh)
    {
        ...
        constraint_goal_sub_ = nh_.subscribe("/camera_node_bottom/target_pose_command", 1, &ObjectPositioning::constraintCommand, this);
                                ...
                                if(!nextPoint)
                                {
                                    if(epsilon)
                                    {
                                        if(sleep)
                                        {
                                            ...
                                            {
                                                dynamixel_command_.data[0] = initDynamixelId1; //mm
                                                dynamixel_command_.data[1] = initDynamixelId2 + 15 * motorRes2; //mm
                                                dynamixel_command_.data[2] = 5;
                                                dynamixel_command_.data[3] = 5;
                                                dynamixel_pub_.publish(dynamixel_command_);
                                                count_l = 0;
    ...
    void orientationCB(const teme_msgs::PoseArrayConstPtr& msg)
    {
        ...
        if(goal_status == 8)
        {
            ...
            polar_queue_.push(std::make_pair(4, 0.0));
                angle_queue_.push(0);
                record_goal.push_back(polar_queue_.back());
            polar_queue_.push(std::make_pair(4, 25));
                angle_queue_.push(30);
                record_goal.push_back(polar_queue_.back());
            polar_queue_.push(std::make_pair(2, -25));
                angle_queue_.push(-30);
                record_goal.push_back(polar_queue_.back());
            polar_queue_.push(std::make_pair(0, -25));
                angle_queue_.push(0);
                record_goal.push_back(polar_queue_.back());
    ...
    void constraintCommand(const geometry_msgs::PoseStamped& msg)
    {
        try
        {
            listener_.waitForTransform(kuka_fixed_frame_, camera_frame_bottom_, ros::Time(0), ros::Duration(0.5));
            listener_.transformPose(kuka_fixed_frame_, msg, constraint_goal);

            ROS_INFO("********Reference is obtained***********");
            ROS_INFO("[Constraint Goal] x:%f, y:%f, z:%f",
                                   constraint_goal.pose.position.x, constraint_goal.pose.position.y, constraint_goal.pose.position.z);

            init_command_ = true;
            prev_ = microrobot_pose;
            double lFound = sqrt(pow(microrobot_pose.pose.position.x - constraint_goal.pose.position.x, 2) +
                                 pow(microrobot_pose.pose.position.y - constraint_goal.pose.position.y, 2)) * 1e+3; //length of the reference(mm)
            double thetaFound = (atan2(constraint_goal.pose.position.y - microrobot_pose.pose.position.y,
                                       constraint_goal.pose.position.x - microrobot_pose.pose.position.x))*180.0/M_PI;

            ROS_INFO("lFound:%f, thetaFound:%f", lFound, thetaFound);

            polar_queue_.push(std::make_pair(lFound, 0.0));
                angle_queue_.push(thetaFound);
            polar_queue_.push(std::make_pair(lFound, 0.0));
                angle_queue_.push(0);

            goal_status = Status::CONSTRAINT;
        ...
        {
            ROS_ERROR("Error in constraint callback:%s" , ex.what());
        }
    }
...
private:
    ...
    ros::NodeHandle nh_;
    ros::Subscriber sub_, constraint_goal_sub_;
    ...
    geometry_msgs::PoseStamped obj_fixed_frame, obj_end_effector, constraint_goal;
