...
void callback(const ObjectPositionMsgConstPtr& obj_xy, const ObjectPositionMsgConstPtr& obj_yz, const sensor_msgs::JointStateConstPtr& kuka)
{
    x = obj_xy->position.data[0]*scale_bottom; //unit mm
    y = obj_xy->position.data[1]*scale_bottom; //unit mm
    z = obj_yz->position.data[1]*scale_front;
    ROS_INFO("X val:%f, Y val:%f, Z val:%f", x,y,z);
    ...
    if(init)
    {
        ...
        if(std::abs(z - center_z) > 0.2) // Z axis unit mm
        {
            double kp = 0.6;
            ...
            cartPos.data[2] = kuka->position[2]; //unit mm
...
int main(int argc, char *argv[]) {
    ...
    ros::Subscriber roi_sub_ = nh.subscribe("/camera_node_front/image_raw_roi",1,&roiCb);
    ...
    Synchronizer<MySyncPolicy> sync(MySyncPolicy(14), xySub, yzSub, kukaSub);
